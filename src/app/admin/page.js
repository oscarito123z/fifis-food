...
export default function AdminPanel() {
  ...
  // FUNCIÓN PARA EL SWITCH MAESTRO
    ...
    <div style={{ backgroundColor: '#000', color: '#fff', minHeight: '100vh', padding: '30px' }}>
      ...
      <div style={{ backgroundColor: '#111', padding: '25px', borderRadius: '25px', border: '2px solid #222', marginBottom: '30px', textAlign: 'center' }}>
        ...
        <button 
          ...
          style={{ width: '100%', backgroundColor: abiertoManual ? '#28a745' : '#d50000', color: '#fff', padding: '20px', borderRadius: '15px', border: 'none', fontWeight: '900', fontSize: '18px' }}
        >
          {abiertoManual ? '🟢 ESTAMOS ABIERTOS' : '🔴 CERRADO MANUALMENTE'}
        </button>
        <p style={{ fontSize: '11px', color: '#555', marginTop: '10px' }}>Si lo ponés en CERRADO, la web dirá que no atendés aunque sea la hora de abrir.</p>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
        {productos.map(p => (
          <div key={p.id} style={{ backgroundColor: '#111', padding: '20px', borderRadius: '20px', border: `1px solid ${p.agotado ? '#ff4444' : '#222'}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <h3 style={{ margin: 0, fontSize: '18px' }}>{p.nombre}</h3>
              <p style={{ margin: 0, color: p.agotado ? '#ff4444' : '#28a745', fontSize: '12px', fontWeight: 'bold' }}>{p.agotado ? '● AGOTADO' : '● DISPONIBLE'}</p>
            </div>
            <button onClick={() => cambiarEstadoProducto(p.id, p.agotado)} style={{ backgroundColor: p.agotado ? '#28a745' : '#ff4444', color: '#fff', border: 'none', padding: '12px 20px', borderRadius: '12px', fontWeight: 'bold' }}>
