...
export default function AdminPanel() {
  ...
  const cargarVentas = async () => {
    ...
    if (data) {
      ...
      data.forEach(v => {
        ...
        let clave = (filtroTiempo === 'dia') ? fechaVenta.toLocaleTimeString([], { hour: '2-digit', minute: '00' }) :
                    (filtroTiempo === 'semana') ? fechaVenta.toLocaleDateString('es-NI', { weekday: 'short' }) :
                    fechaVenta.toLocaleDateString('es-NI', { day: '2-digit', month: 'short' });
  ...
  const manejarSubidaImagen = async (e) => {
    try {
      ...
      setFormEdit({ ...formEdit, imagen: publicUrlData.publicUrl });
  ...
  return (
    <div style={{ backgroundColor: '#000', color: '#fff', minHeight: '100vh', padding: '20px', paddingBottom: '100px' }}>
      {/* ... (Cabecera y Gráfica se mantienen igual) ... */}
      ...
      <div style={{ backgroundColor: '#111', padding: '20px', borderRadius: '25px', border: '1px solid #222', marginBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '15px' }}>
          <div><p style={{ fontSize: '12px', color: '#777', margin: 0 }}>Venta Total ({filtroTiempo})</p><h3 style={{ color: '#00c853', margin: 0, fontSize: '22px' }}>C$ {ventaTotalPeriodo}</h3></div>
        ...
        <div style={{ width: '100%', height: 180 }}>
          <ResponsiveContainer>
            {tipoGrafica === 'lineas' ? (
              <LineChart data={datosGrafica}><CartesianGrid strokeDasharray="3 3" stroke="#222" /><XAxis dataKey="name" stroke="#555" fontSize={10} /><Tooltip contentStyle={{ backgroundColor: '#000', border: '1px solid #333' }} /><Line type="monotone" dataKey="ventas" stroke="#FF8C00" strokeWidth={3} dot={false} /></LineChart>
            ) : (
              <BarChart data={datosGrafica}><CartesianGrid strokeDasharray="3 3" stroke="#222" /><XAxis dataKey="name" stroke="#555" fontSize={10} /><Tooltip contentStyle={{ backgroundColor: '#000', border: '1px solid #333' }} /><Bar dataKey="ventas" fill="#FF8C00" radius={[4, 4, 0, 0]} /></BarChart>
            )}
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: '8px', overflowX: 'auto', marginBottom: '20px', paddingBottom: '10px' }}>
        {categorias.map(cat => {
          ...
          return (
            <button key={cat} onClick={() => toggleCategoriaCompleta(cat)} style={{ backgroundColor: estaApagada ? '#330000' : '#003300', border: `1px solid ${estaApagada ? '#ff4444' : '#00c853'}`, color: '#fff', padding: '10px 15px', borderRadius: '15px', whiteSpace: 'nowrap', fontSize: '11px', fontWeight: 'bold' }}>{estaApagada ? `❌ ${cat}` : `✅ ${cat}`}</button>
          );
        })}
      </div>

      <button onClick={toggleEstadoLocal} style={{ width: '100%', backgroundColor: abiertoManual ? '#28a745' : '#d50000', color: '#fff', padding: '15px', borderRadius: '15px', fontWeight: 'bold', border: 'none', marginBottom: '20px' }}>{abiertoManual ? '🟢 LOCAL ABIERTO' : '🔴 LOCAL CERRADO'}</button>
          ...
          <div key={p.id} style={{ backgroundColor: '#111', padding: '15px', borderRadius: '25px', border: `1px solid ${p.agotado ? '#ff4444' : '#222'}` }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                ...
                <div><h4 style={{ margin: 0, fontSize: '14px' }}>{p.nombre}</h4><p style={{ margin: 0, color: '#FF8C00', fontSize: '12px', fontWeight: 'bold' }}>C$ {p.precio}</p></div>
              </div>
              <button onClick={() => cambiarEstadoProducto(p.id, p.agotado)} style={{ backgroundColor: p.agotado ? '#28a745' : '#333', color: '#fff', border: 'none', padding: '8px 12px', borderRadius: '10px', fontSize: '10px', fontWeight: 'bold' }}>{p.agotado ? 'ACTIVAR' : 'AGOTAR'}</button>
            </div>
            ...
            {editandoId === p.id ? (
              <div style={{ marginTop: '15px', borderTop: '1px solid #222', paddingTop: '15px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                ...
                {/* NUEVO: BOTÓN DE SUBIDA DIRECTA */}
                <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                  <label style={{ fontSize: '10px', color: '#777' }}>Actualizar Foto:</label>
                  <input type="file" accept="image/*" onChange={manejarSubidaImagen} disabled={subiendo} style={{ fontSize: '11px' }} />
                  {subiendo && <p style={{ fontSize: '10px', color: '#FF8C00' }}>Subiendo archivo...</p>}
                  <input type="text" value={formEdit.imagen} readOnly style={{ backgroundColor: '#050505', border: '1px solid #222', padding: '8px', borderRadius: '8px', color: '#555', fontSize: '9px' }} />
                </div>
              ...
              <button onClick={() => { setEditandoId(p.id); setFormEdit({ precio: p.precio, desc: p.desc, imagen: p.imagen }); }} style={{ width: '100%', background: 'none', border: '1px solid #222', color: '#555', padding: '8px', borderRadius: '10px', fontSize: '11px', marginTop: '10px' }}>⚙️ EDITAR DETALLES</button>
            )}
          </div>
        ))}
      </div>
